	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
				...
				WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "codex_models", "codex_degraded", "codex_last_292", "codex_last_312"}).
...
func TestLockAndMergeAccountProbeExtraPreservesCodexRuntimeState(t *testing.T) {
	db, mock, err := sqlmock.New()
	require.NoError(t, err)
	t.Cleanup(func() { _ = db.Close() })
	client := dbent.NewClient(dbent.Driver(entsql.OpenDB(dialect.Postgres, db)))
	...
	databaseModels := []byte(`{"gpt-6-astra":{"degraded":true}}`)
	mock.ExpectQuery(`(?s)`+regexp.QuoteMeta("SELECT")+`.*`+regexp.QuoteMeta("FOR NO KEY UPDATE")).
		WithArgs(int64(42), service.PlatformOpenAI, service.AccountTypeOAuth, `{"access_token":"token"}`, nil).
		WillReturnRows(sqlmock.NewRows([]string{"identity_unchanged", "ollama_group_unchanged", "ollama_proxy_unchanged", "enabled", "rate_sync_enabled", "snapshot", "ollama_session", "ollama_auto", "ollama_snapshot", "codex_models", "codex_degraded", "codex_last_292", "codex_last_312"}).
			AddRow(true, false, true, nil, nil, nil, nil, nil, nil, databaseModels, []byte(`true`), nil, []byte(`"2026-09-17T12:00:00Z"`)))
	...
	account := &service.Account{
		...
		Extra: map[string]any{
			service.CodexStateModelsExtraKey:   map[string]any{"forged": map[string]any{"degraded": false}},
			service.CodexStateDegradedExtraKey: false,
			service.CodexStateLast312ExtraKey:  "forged",
		},
	}
	got, err := lockAndMergeAccountProbeExtra(context.Background(), client, account, nil, nil)
	require.NoError(t, err)
	require.Equal(t, map[string]any{"gpt-6-astra": map[string]any{"degraded": true}}, got[service.CodexStateModelsExtraKey])
	require.Equal(t, true, got[service.CodexStateDegradedExtraKey])
	require.Equal(t, "2026-09-17T12:00:00Z", got[service.CodexStateLast312ExtraKey])
	require.NotContains(t, got, service.CodexStateLast292ExtraKey)
	require.NoError(t, mock.ExpectationsWereMet())
}
